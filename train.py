...
def train(config, model, device, train_loader_faces, train_loader_cartoons, optimizers, criterion_bc, criterion_l1, criterion_l2):
    ...
    for faces_batch, cartoons_batch in zip(train_loader_faces, train_loader_cartoons):
        ...
        loss_disc1_real_cartoons = config.wGan_loss * \
            criterion_bc(output_real.squeeze(), torch.ones_like(
                output_real.squeeze(), device=device))
        ...
        loss_disc1_fake_cartoons = config.wGan_loss * \
            criterion_bc(output_fake.squeeze(), torch.zeros_like(
                output_fake.squeeze(), device=device))
...
def model_train(config_file, use_wandb=True):
    ...
    train_loader_faces, test_loader_faces, train_loader_cartoons, test_loader_cartoons = get_datasets(
        config)
    ...
    images_faces_to_test = get_test_images(
        config, config.root_path + config.dataset_path_test_faces, config.root_path + config.dataset_path_segmented_faces)

    for epoch in tqdm(range(config.num_epochs)):
        loss_rec1, loss_rec2, loss_dann, loss_sem1, loss_sem2, loss_disc1, loss_gen1, loss_total, loss_denoiser, loss_teach, loss_disc1_real_cartoons, loss_disc1_fake_cartoons = train(
            config, model, device, train_loader_faces, train_loader_cartoons, optimizers, criterion_bc, criterion_l1, criterion_l2)
